...
- **B5** 하이브리드 검색 — **ES BM25 + kNN RRF** 결합 (반드시 B2 하네스로 개선 증명). ~~MySQL fulltext~~는 C1(ES 이행, 2026-07-12) 이전에 쓴 계획 — C1이 ES를 고른 근거 자체가 "B5가 같은 엔진에서 자연 확장"이었다. **정량적 동기 (2026-07-16)**: 멀티턴 재작성이 만든 흠 없는 질의 `"연차 휴가는 언제까지 사용해야 하나요?"`가 `vacation`을 **5위/8**에 둔다(design-notes §7 "판정 — 9번째 쌍"). **착수 전 선검증 완료 (2026-07-16, 제품 코드 0 — ES에 코퍼스만 넣고 BM25 질의)**: 같은 질의가 **BM25에선 1위**(가설 통과) · 문맥 의존 원문 `"그럼 언제까지 써야 하나요?"`는 **BM25 0건**인데 벡터는 7위로라도 찾는다 → **두 방식이 서로 다른 실패를 한다 = RRF의 존재 이유가 실측으로 확인됨**. ⚠️ 단 과잉 특정 X안도 BM25 1위라 **"B5 성공 = 9번째 쌍 편입 가능"은 오판이었다**(하이브리드에서도 O안·X안 둘 다 HIT → 판별력 0. 같은 날 START-HERE §5-1에 적었다가 이 프로브로 정정). **착수 전 함정 검증 (2026-07-16, C2-1·C4와 같은 규율)**:
    - 🚧 **네이티브 RRF는 못 쓴다** — compose 기본 이미지는 **basic 라이선스**이고 `retriever.rrf` 실호출이 `security_exception: current license is non-compliant for [Reciprocal Rank Fusion (RRF)]`로 거부된다(실측). → **RRF를 직접 구현**해야 한다(공식은 `score = Σ 1/(k + rank_i)`, k=60 관례로 단순). 부수 효과: **"유료라 못 썼고, 공식이 단순해 직접 구현했다 + 원리를 설명할 수 있다"**가 D안 Kafka·Spring AI VectorStore와 같은 서사 구조로 남는다.
    - 🚧 **nori(한국어 형태소 분석기)는 8.x 기본 미포함**(`_cat/plugins` 빈 결과). 설치하려면 커스텀 이미지 빌드가 필요하고 **Testcontainers도 같은 이미지를 써야 한다**(테스트 인프라 변경 = C1 2-2 계열 비용).
    - **분석기 프로브 실측**: 기본(standard)은 공백 분절(`연차 | 휴가를 | 언제까지 | 사용해야 | 하나요`)인데도 조사 변형 4종에서 전부 `vacation` 1위였다 — `연차`가 조사 없이 토큰으로 존재해서다. **문제는 조사가 아니라 recall**: 후보가 1~2건뿐이고 문맥 의존 원문엔 0건이라 **BM25 쪽 순위 목록이 희박해 RRF로 합칠 재료가 안 된다**. nori의 값어치는 여기서 판단해야 하며, 넣고/빼고를 B2 하네스로 비교하는 것이 결정 방식.

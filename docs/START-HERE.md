...
- 🔧 **Phase C1·C4·C3 - 채용 공고 대응 딥다이브 (2026-07-07 편성, 2026-07-11 강의 접목 보강)**: C1 벡터 DB 이행(B1 패턴 이식·B2 검증) → **C4 ask-wiki MCP 서버(신설)** → C3 에이전틱 RAG. 근거·매핑은 ROADMAP "채용 공고 대응" 참조. (C2는 위로 승격·완료.)
    ...
    - **✅ Step 2-1 완료 (2026-07-12)**: `VectorIndex` 포트 추출(동작 불변) — Codex 작성, Claude 검증(diff 리뷰 + 전체 19 테스트 그린) 후 커밋 `635431b`. Codex 샌드박스가 gradle 소켓/락을 막아 **테스트 실행은 Claude 몫**(B1 3-7과 같은 제약 패턴). **지금 여기: Step 2-2(ES compose 컨테이너 + 클라이언트 의존성 + 연결 스모크 테스트) — Codex 위임 진행.**

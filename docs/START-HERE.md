...
- 🔧 **Phase C2 - 상용 LLM 프로바이더 스위치 (계획 확정, 착수 대기, 2026-07-10)**: B2가 정량적 동기 제공 — 로컬 소형 모델의 환각 바닥(65%/60%)을 상용 모델이 깨는지 B2 하네스로 증명. **확정 결정**: (1) 세 개(Claude/GPT/Gemini) 모두 스위치 가능하게(프로퍼티 `askwiki.llm.provider`), 우선 1개 붙이고 확장 (2) **챗 LLM만 교체**·임베딩 nomic 유지(→ 환각↓ 원인을 LLM 판단으로 귀속하는 실험 통제) (3) B3(SSE) 미포함. 서브스텝 C2-1~C2-5(+배포)는 ROADMAP "Phase C2". 구 B4(장애 격리)·배포 과제 흡수.
    ...
    - **지금 여기 (2026-07-10)**: C2-1·C2-2 완료. 측정 순서 = **① Gemini 무료(1순위)** → 불만족 시 **② Claude Sonnet 5**. 다음 = **C2-3**(토큰·비용·지연 Micrometer 지표 + Grafana). 그다음 C2-4(장애격리)·C2-5(프로바이더별 환각률 비교). ⚠️ 임베딩은 Gemini 써도 로컬 Ollama(nomic)라 실질의·측정엔 Ollama 가동 필수. 변경은 작업 트리 미커밋(연우님 리뷰·커밋). **✅ Gemini 무료 경로 스모크 검증 완료**(2026-07-10): OpenAI-compat 엔드포인트에 `.env` 키로 직접 curl → HTTP 200·한국어 정상 응답·`reasoning_effort:none`으로 thinking 끄니 12s→1s·사고 토큰 0. → application.yml에 `spring.ai.openai.chat.options.reasoning-effort: none` 반영. 변경은 작업 트리 미커밋(연우님 리뷰·커밋 예정).

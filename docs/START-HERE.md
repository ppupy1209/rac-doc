...
- 🔧 **Phase C2 - 상용 LLM 프로바이더 스위치 (계획 확정, 착수 대기, 2026-07-10)**: B2가 정량적 동기 제공 — 로컬 소형 모델의 환각 바닥(65%/60%)을 상용 모델이 깨는지 B2 하네스로 증명. **확정 결정**: (1) 세 개(Claude/GPT/Gemini) 모두 스위치 가능하게(프로퍼티 `askwiki.llm.provider`), 우선 1개 붙이고 확장 (2) **챗 LLM만 교체**·임베딩 nomic 유지(→ 환각↓ 원인을 LLM 판단으로 귀속하는 실험 통제) (3) B3(SSE) 미포함. 서브스텝 C2-1~C2-5(+배포)는 ROADMAP "Phase C2". 구 B4(장애 격리)·배포 과제 흡수.
    ...
    - **지금 여기 (2026-07-12)**: C2-5 잔여 = **Gemini 측정, 다음 세션 첫 결정: 3.x 무료(gemini-3.5-flash→3.1-flash-lite 폴백) vs 유료 Tier 1**(2026-07-12 연우님 "오늘은 여기까지"로 보류). 실행법은 아래 착수 메모 + 추가 env: `ASKWIKI_LLM_CALL_TIMEOUT_MS=120000`(양 프로바이더 공통)·Gemini는 `ASKWIKI_EVAL_PACING_MS=13000`·모델 오버라이드는 `SPRING_AI_OPENAI_CHAT_OPTIONS_MODEL=<모델명>`. ⚠️ 임베딩은 Gemini 써도 로컬 Ollama(nomic)라 측정엔 Ollama 가동 필수.

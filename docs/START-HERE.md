...
- 🔧 **Phase C1·C4·C3 - 채용 공고 대응 딥다이브 (2026-07-07 편성, 2026-07-11 강의 접목 보강)**: C1 벡터 DB 이행 ✅ → **C4 ask-wiki MCP 서버(신설, 다음)** → C3 에이전틱 RAG. 근거·매핑은 ROADMAP "채용 공고 대응" 참조. (C2는 위로 승격·완료.)
    - **✅ C1 완료 (2026-07-12)**: ES kNN 이행 — 자체 포트 `VectorIndex`+ES 어댑터(멱등 `_id`·kNN·점수 역변환), B1 정합성 4종 ES 재증명, 측정 5종(위). **지금 여기 = 다음 Phase C4(ask-wiki를 MCP 서버로 노출: `search_wiki`·`ask_wiki` tool, `spring-ai-starter-mcp-server-webmvc` — Boot 3.3+·1.0.x 포함 확인, C2-1식 함정 없음). 착수 전 인프런 강의 섹션 10 수강 권장.** 구현은 Codex 위임·측정/설계는 Claude(현 세션 확립 패턴).

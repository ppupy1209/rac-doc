...
- 🔧 **Phase C1·C4·C3 - 채용 공고 대응 딥다이브 (2026-07-07 편성, 2026-07-11 강의 접목 보강)**: C1 벡터 DB 이행 ✅ → **C4 ask-wiki MCP 서버(신설, 다음)** → C3 에이전틱 RAG. 근거·매핑은 ROADMAP "채용 공고 대응" 참조. (C2는 위로 승격·완료.)
    ...
    - **✅ C3-1 구현 (2026-07-14)**: `WikiSearchTool`(per-call 도구·소스 수집기, 전역 빈 아님)·`AgenticRagService`(ChatClient 도구 루프 → RagResult, 단발 `RagService` 유지). 결정적 단위테스트 5종(fallback — 목 ChatModel은 Spring AI 내부 tool 루프를 못 돌려서 도구·매핑을 분리 테스트). **전체 스위트 44 그린**(컨텍스트 부팅 OK, C4식 순환 없음). Gemini tool-calling 선검증 통과. Codex 위임·Claude 검증. **지금 여기 = C3-1 측정**(단발 vs 에이전틱 B2 on Gemini + 멀티홉 golden 추가).
